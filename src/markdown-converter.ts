...
export class MarkdownConverter {
  ...
  async markdownToConfluence(markdown: string): Promise<string> {
    ...
    // リンク参照形式を展開
    const expandedContent = this.expandLinkReferences(content);
  ...
  private simpleMarkdownToHtml(markdown: string): string {
    ...
    let inAlert: string | false = false; // false | 'NOTE' | 'WARNING' | 'TIP' | 'IMPORTANT' | 'CAUTION'
    ...
    for (let i = 0; i < lines.length; i++) {
      ...
      if (orderedMatch && !inTable) {
        const content = this.processInlineMarkdown(orderedMatch[2] || '');
  ...
  private processInlineMarkdown(text: string): string {
    ...
    // オートリンク: <URL> （HTMLエスケープ前にプレースホルダーに変換）
    text = text.replace(/<(https?:\/\/[^\s>]+)>/g, '[[AUTOLINK:$1]]');
    text = text.replace(/<([^\s@>]+@[^\s>]+)>/g, '[[MAILTO:$1]]');

    // HTML特殊文字のエスケープ（実体参照以外）
    ...
    // 太字斜体の組み合わせ（***text*** or ___text___）- 先に処理
    ...
    // 斜体（*text*）
    text = text.replace(/\*([^*\n]+)\*/g, '<em>$1</em>');
    // アンダースコア斜体（_text_）- 単語境界を考慮してsnake_caseを保護
