...
echo ===============================================================================
echo      Confluence MCP Server DataCenter版 16 APIs 完全検証
echo ===============================================================================
...
echo.
echo Testing API 1/16: get_current_user
...
echo.
echo Testing API 7/16: update_page
echo {"jsonrpc":"2.0","id":7,"method":"tools/call","params":{"name":"confluence_update_page","arguments":{"id":163841,"title":"Updated DataCenter Test","body":{"storage":{"value":"<p>Updated via DataCenter API</p>","representation":"storage"}},"version":{"number":3}}}} | node build/index.js
if !ERRORLEVEL! EQU 0 (echo ✅ SUCCESS && set /a SUCCESS+=1) else (echo ❌ FAIL && set /a FAIL+=1)

echo.
echo Testing API 8/16: delete_page (test page)
echo {"jsonrpc":"2.0","id":8,"method":"tools/call","params":{"name":"confluence_delete_page","arguments":{"id":163946}}} | node build/index.js
if !ERRORLEVEL! EQU 0 (echo ✅ SUCCESS && set /a SUCCESS+=1) else (echo ❌ FAIL && set /a FAIL+=1)

echo.
echo Testing API 9/16: search_content (NEW - CQL Search)
...
echo.
echo Testing API 10/16: get_content_labels (NEW)
...
echo.
echo Testing API 11/16: add_content_label (NEW)
...
echo.
echo Testing API 12/16: get_user_by_id
...
echo.
echo Testing API 13/16: get_users (NEW - User Search)
...
echo.
echo Testing API 14/16: page_to_markdown
...
echo.
echo Testing API 15/16: markdown_to_page
...
echo.
echo Testing API 16/16: export_space_to_markdown
echo {"jsonrpc":"2.0","id":16,"method":"tools/call","params":{"name":"confluence_export_space_to_markdown","arguments":{"spaceId":131083,"outputDir":"./datacenter-export-test"}}} | node build/index.js
if !ERRORLEVEL! EQU 0 (echo ✅ SUCCESS && set /a SUCCESS+=1) else (echo ❌ FAIL && set /a FAIL+=1)

echo.
echo ===============================================================================
echo             DataCenter版 16 API 完全検証結果
echo ===============================================================================
echo Completion Time: %date% %time%
echo.
echo 📊 DataCenter対応統計:
echo ✅ 動作確認済みAPI:        %SUCCESS%/16
echo ❌ 失敗API:               %FAIL%/16
echo.
echo 🎯 DataCenter版機能分析:
if %SUCCESS% GEQ 13 (
    echo   Status: 🎉 EXCELLENT - DataCenter完全対応
) else if %SUCCESS% GEQ 10 (
    echo   Status: 🔶 GOOD - ほぼ完全対応
) else (
    echo   Status: ⚠️ NEEDS WORK - 要改善
)
echo.
echo ✅ DataCenter対応済み16APIs:
echo   【基本機能】
...
echo   16. export_space_to_markdown - スペース一括エクスポート
echo.
